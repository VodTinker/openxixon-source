...
async function fetchIncidents(key: string, label: string): Promise<Array<{ label: string; start: string; end: string | null }>> {
  try {
    ...
    const results: GatusResult[] = (data.results ?? [])
      .slice()
      .sort((a: GatusResult, b: GatusResult) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime())
...
function mergeIncidents(raw: Array<{ label: string; start: string; end: string | null }>): Incident[] {
...
export const GET: APIRoute = async () => {
  ...
  try {
    ...
    // Usar ep.key real de Gatus (incluye prefijo _ cuando no hay grupo)
    ...
    return new Response(JSON.stringify({ ts: new Date().toISOString(), overall, datasets: datasetResults, endpoints: endpointResults, incidents }), {
      headers: { 'Content-Type': 'application/json', 'Cache-Control': 'public, s-maxage=30, stale-while-revalidate=60' },
    })
  } catch {
    // Gatus no disponible — fallback a Supabase directo
  }
